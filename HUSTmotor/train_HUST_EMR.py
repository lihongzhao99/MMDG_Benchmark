...
from models import ERM_Model as models
...
RESULT_LOG_DIR = osp.join(BASE_DIR, 'outputs', 'logs', 'ERM')
...
def train(model, src_loader, val_loader, target_loaders, iteration, lr, cuda, task_name):
    ...
    for i in pbar:
        ...
        src_pred3 = model(vib_data, aud_data)

        loss = F.nll_loss(F.log_softmax(src_pred3, dim=1), cls_label)
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    model = models.ERM(num_classes=class_num)
    if cuda:
        model.cuda()
