...
from models import MBCD_Model as models
...
RESULT_LOG_DIR = osp.join(BASE_DIR, 'outputs', 'logs', 'MBCD')
...
def update_ema(model, model_ema, ema_beta):
    with torch.no_grad():
        for param_q, param_k in zip(model.parameters(), model_ema.parameters()):
            param_k.data.mul_(ema_beta).add_(param_q.data, alpha=1 - ema_beta)
...
def train(model, src_loader, val_loader, target_loaders, iteration, lr, cuda, task_name, args):
    ...
    for i in pbar:
        ...
        preds = model(vib_data, aud_data, modality_drop_base=args.modality_drop_base, return_all_logits=True)
        
        loss_v = F.cross_entropy(preds['vibration'], cls_label)
        loss_a = F.cross_entropy(preds['sound'], cls_label)
        loss_uni = loss_v + loss_a

        loss_uni.backward()

        original_params = {}
        for name, param in model.named_parameters():
            if param.grad is not None and ('sharedNet' in name or 'vib_norm' in name or 'aud_norm' in name):
                original_params[name] = param.data.clone()
                param.data.sub_(alpha * param.grad)

        preds_prime = model(vib_data, aud_data, modality_drop_base=args.modality_drop_base, return_all_logits=True)
        src_pred3_prime = preds_prime['fusion']
        
        loss_mm = F.cross_entropy(src_pred3_prime, cls_label)
        loss_pass2 = loss_mm

        if args.kl_mm_coeff != 0 or args.kl_um_coeff != 0:
            with torch.no_grad():
                preds_ema = model_ema(vib_data, aud_data, return_all_logits=True)
                mm_prob_ema = F.softmax(preds_ema['fusion'], dim=-1).clamp_min(1e-12)

            if args.kl_mm_coeff != 0:
                mm_log_prob = F.log_softmax(src_pred3_prime, dim=-1)
                kd_loss_mm = F.kl_div(mm_log_prob, mm_prob_ema, reduction='batchmean')
                loss_pass2 += kd_loss_mm * args.kl_mm_coeff

            if args.kl_um_coeff != 0:
                v_log_prob = F.log_softmax(preds_prime['vibration'], dim=-1)
                a_log_prob = F.log_softmax(preds_prime['sound'], dim=-1)
                ...
                loss_pass2 += (kd_loss_v + kd_loss_a) * args.kl_um_coeff
        ...
        update_ema(model, model_ema, args.ema_beta)

        total_loss_display = loss_uni.item() + loss_pass2.item()
        
        if i % log_interval == 0:
            pbar.set_postfix({
                'Loss': f'{total_loss_display:.4f}',
                'BestVal': f'{best_val_acc:.2f}%'
            })

        if i % (log_interval * 20) == 0:
            tqdm.write(f"\n[Iter {i}] Validation & Testing...")
            test_source(model_ema, src_loader, cuda)
            current_val_acc = test_validation(model_ema, val_loader, cuda)
            current_per_target, current_test_acc = evaluate_target_domains(model_ema, target_loaders, cuda)
            
            if current_val_acc > best_val_acc:
                best_val_acc = current_val_acc
                best_test_acc_at_val = current_test_acc
                best_per_target_at_val = current_per_target
                torch.save(model_ema.state_dict(), osp.join(CKPT_DIR, f'best_model_{task_name}.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha', type=float, default=0.001, help='Inner-loop learning rate')
    ...
    model = models.MBCD(num_classes=class_num, hidden_dim=args.hidden_dim)
    if cuda:
        model.cuda()

    best_val_acc, test_at_best_val, best_per_target_at_val = train(
        model, src_loader, val_loader, target_loaders, iteration, lr, cuda, task_name, args
    )
