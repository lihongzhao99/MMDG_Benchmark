...
from models import RNA_Model as models
...
RESULT_LOG_DIR = osp.join(BASE_DIR, "outputs", "logs", "RNA")
CKPT_DIR = osp.join(BASE_DIR, "outputs", "models", "RNA")
...
def test_validation(model, val_loader, cuda):
    ...
    with torch.no_grad():
        for val_data, val_label in val_loader:
            ...
            pred3, _, _ = model(vib_data, aud_data)
...
def train(model, src_loader, val_loader, target_loaders, iteration, lr, cuda, task_name, alpha_RNA=1.0):
    ...
    for i in pbar:
        ...
        optimizer = torch.optim.Adam([
            ...
            {'params': model.fusion_cls.parameters(), 'lr': LEARNING_RATE},
        ...
        src_pred3, vibration_feature, sound_feature = model(vib_data, aud_data)

        vibration_norm = vibration_feature.norm(p=2, dim=1).mean()
        sound_norm = sound_feature.norm(p=2, dim=1).mean()
        feat_frac = vibration_norm / sound_norm
        loss_RNA = (feat_frac - 1) ** 2

        loss = criterion_cls(src_pred3, cls_label) + alpha_RNA * loss_RNA
        loss.backward()
        optimizer.step()
        ...
        if i % (log_interval * 20) == 0:
            ...
            if current_val_acc > best_val_acc:
                ...
                tqdm.write(f">>> New Best Val: {best_val_acc:.2f}% | MeanTest@BestVal: {best_test_acc_at_val:.2f}% (Model Saved)")
...
def test_target(model, test_loader, cuda, title="Target"):
    ...
    with torch.no_grad():
        for tgt_test_data, tgt_test_label in test_loader:
            ...
            tgt_pred3, _, _ = model(vib_data, aud_data)
...
if __name__ == "__main__":
    ...
    model = models.BASE(num_classes=class_num)
    if cuda:
        model.cuda()
