...
from models import NEL_Model as models
...
RESULT_LOG_DIR = osp.join(BASE_DIR, 'outputs', 'logs', 'NEL')
...
class ConMeanShiftLoss(nn.Module):
    """Unsupervised contrastive loss between two views."""
    ...
    def forward(self, view1, view2):
        ...
        logits_12 = torch.matmul(view1, view2.t()) / self.temperature
        logits_21 = torch.matmul(view2, view1.t()) / self.temperature
...
def compute_mean_shift_embedding(embeddings, k=8):
    """Mean-shift embedding from NEL (Eq. 4-6)."""
    n = embeddings.size(0)
    if n < 2:
    ...
    _, knn_idx = torch.topk(sim, k=k_eff + 1, dim=1, largest=True)
    knn_idx = knn_idx[:, 1:]

    neighbor_weight = 0.5 / k_eff
...
def compute_nee_loss(embeddings):
    """Von Neumann entropy objective from NEL (Eq. 11-12)."""
    n = embeddings.size(0)
    c_auto = torch.matmul(embeddings.t(), embeddings) / max(n, 1)
...
def train(model, src_loader, val_loader, target_loaders, iteration, lr, cuda, task_name,
          alpha=0.7, beta=None, temp_s=0.1, temp_u=0.25, k=8):
    ...
    if beta is None:
        beta = 1.0 / src_loader.batch_size

    optimizer = torch.optim.Adam(
        model.parameters(), lr=lr, weight_decay=l2_decay
    )
    criterion_cls = nn.CrossEntropyLoss()
    criterion_supcon = SupConLoss(temperature=temp_s)
    criterion_unsupcon = ConMeanShiftLoss(temperature=temp_u)

    if cuda:
        criterion_cls = criterion_cls.cuda()
        criterion_supcon = criterion_supcon.cuda()
        criterion_unsupcon = criterion_unsupcon.cuda()
    ...
    for i in pbar:
        ...
        src_pred3, vib_emd, aud_emd, vib_proj, aud_proj, fused_emd = model(
            vib_data, aud_data, return_features=True
        )

        cls_loss = criterion_cls(src_pred3, cls_label)

        # L_C: supervised contrastive on initial projected embeddings
        emd_proj = torch.stack([vib_proj, aud_proj], dim=1)
        loss_supervised = criterion_supcon(emd_proj, cls_label)

        # L_UNC: unsupervised contrastive on mean-shift embeddings
        ms_v = compute_mean_shift_embedding(vib_proj, k=k)
        ms_a = compute_mean_shift_embedding(aud_proj, k=k)
        loss_unsupervised = criterion_unsupcon(ms_v, ms_a)

        # L_NEE: Von Neumann entropy regularisation on fused embedding
        loss_nee = compute_nee_loss(fused_emd)

        # Eq. 14: L = cls + alpha * L_UNC + (1-alpha) * L_C + beta * L_NEE
        loss = cls_loss + alpha * loss_unsupervised + (1.0 - alpha) * loss_supervised + beta * loss_nee
        loss.backward()
        optimizer.step()
        ...
        if i % log_interval == 0:
            pbar.set_postfix({
                ...
                'UNC': f'{loss_unsupervised.item():.4f}',
                'C': f'{loss_supervised.item():.4f}',
                'NEE': f'{loss_nee.item():.4f}',
...
if __name__ == '__main__':
    ...
    model = models.NEL(num_classes=class_num)
    if cuda:
        model.cuda()
    ...
    best_val_acc, test_at_best_val, best_per_target_at_val = train(
        ...
        alpha=alpha, beta=beta, temp_s=temp_s, temp_u=temp_u, k=k
    )
