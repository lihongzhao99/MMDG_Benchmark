...
class HACDOMAIN(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_audio:
            audio_path = self.base_path + 'HAC_Splits/' + self.prefix_list[index] + 'audio/' + self.video_list[index][:-4] + '.wav'
            if self.use_video:
        ...
        domain_key = self.prefix_list[index].rstrip('/')
        domain_mapping = {dom: i for i, dom in enumerate(self.domain)}
        mapped_value = domain_mapping.get(domain_key, 0)
