...
class HACDOMAIN(torch.utils.data.Dataset):
    def __init__(self, split='test', source=True, domain=['human'],  modality='rgb', cfg=None, cfg_flow=None, use_video=True, use_flow=True, use_video_corruption=False, use_audio_corruption=False, use_audio=True, datapath='/path/to/HAC_DATA_ROOT'):
        ...
        self.use_video_corruption = use_video_corruption
        self.use_audio_corruption = use_audio_corruption
        ...
        for dom in domain:
            ...
            # else:
            #     prefix = 'cartoon/'
            with open(os.path.join(self.base_path, "HAC_Splits", f"HAC_{split}_only_{dom}.csv")) as f:
                f_csv = csv.reader(f)
            ...
            if split == 'test' and not source:
                with open(os.path.join(self.base_path, "HAC_Splits", f"HAC_train_only_{dom}.csv")) as f:
                    f_csv = csv.reader(f)
                    for i, row in enumerate(f_csv):
                        self.video_list.append(row[0])
                        self.prefix_list.append(prefix)
        ...
        self.interval = 9
        self.video_path_base = os.path.join(self.base_path, 'HAC')
        if not os.path.exists(self.video_path_base):
            os.mkdir(self.video_path_base)

    def __getitem__(self, index):
        label1 = int(self.label_list[index])
        video_path = os.path.join(self.video_path_base, self.video_list[index], '')
        video_path = video_path + self.video_list[index] + '-'

        if self.use_video:
            if self.use_video_corruption:
                video_file = os.path.join(self.base_path, 'HAC-C', self.prefix_list[index], 'videos', 'defocus_blur', self.video_list[index])
            else:
                video_file = os.path.join(self.base_path, self.prefix_list[index], 'videos', self.video_list[index])
        ...
        if self.use_flow:
            video_file_x = os.path.join(self.base_path, self.prefix_list[index], 'flow', self.video_list[index][:-4] + '_flow_x.mp4')
            video_file_y = os.path.join(self.base_path, self.prefix_list[index], 'flow', self.video_list[index][:-4] + '_flow_y.mp4')
        ...
        if self.use_audio:
            if self.use_audio_corruption:
                audio_path = os.path.join(self.base_path, 'HAC-C', self.prefix_list[index], 'audio', 'wind', self.video_list[index][:-4] + '.wav')
            else:
                audio_path = os.path.join(self.base_path, self.prefix_list[index], 'audio', self.video_list[index][:-4] + '.wav')
            # audio_path = os.path.join(self.base_path, 'HAC-C', self.prefix_list[index], 'audio', 'wind', self.video_list[index][:-4] + '.wav')
            # audio_path = os.path.join(self.base_path, self.prefix_list[index], 'audio', self.video_list[index][:-4] + '.wav')
