...
class HACDOMAIN(torch.utils.data.Dataset):
    def __init__(self, split='test', source=True, domain=['human'],  modality='rgb', cfg=None, cfg_flow=None, use_video=True, use_flow=True, use_audio=True, datapath='/path/to/HAC_DATA_ROOT'):
    ...
    def __getitem__(self, index):
        ...
        if self.use_video:
            video_file = os.path.join(self.base_path, 'HAC_Splits', self.prefix_list[index], 'videos', self.video_list[index])
        ...
        if self.use_flow:
            video_file_x = os.path.join(self.base_path, 'HAC_Splits', self.prefix_list[index], 'flow', self.video_list[index][:-4] + '_flow_x.mp4')
            video_file_y = os.path.join(self.base_path, 'HAC_Splits', self.prefix_list[index], 'flow', self.video_list[index][:-4] + '_flow_y.mp4')
        ...
        if self.use_audio:
            audio_path = os.path.join(self.base_path, 'HAC_Splits', self.prefix_list[index], 'audio', self.video_list[index][:-4] + '.wav')
            if self.use_video:
