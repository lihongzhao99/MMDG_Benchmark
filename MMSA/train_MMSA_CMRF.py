...
from models import CMRF_Model
from utils.dataloader import getdataloader
from utils.run_logging import append_run_log


parser = argparse.ArgumentParser(description="MMSA + CMRF domain generalization")
...
parser.add_argument(
    ...
    type=str,
    default="mosi",
    help="single dataset fallback (deprecated, prefer --source_datasets)",
)
parser.add_argument(
    "--source_datasets",
    nargs="+",
...
# ===== CMRF related arguments =====
parser.add_argument(
    "--cmrf_embed_dim",
    type=int,
    default=128,
    help="half of the modality embedding dim; encoder output is 2x this value",
)
parser.add_argument(
    "--cmrf_hidden_dim",
    type=int,
    default=256,
    help="hidden size for CMRF modality encoders and auxiliary heads",
)
parser.add_argument(
    "--cmrf_proj_dim",
    type=int,
    default=128,
    help="projection dim for CMRF supervised contrastive learning",
)
parser.add_argument(
    "--cmrf_mixup",
    action="store_true",
    help="enable cross-modal representation mixup",
)
parser.add_argument(
    "--mix_alpha",
    type=float,
    default=1.0,
    help="beta distribution alpha used for cross-modal mixup",
)
parser.add_argument(
    "--mix_coef",
    type=float,
    default=2.0,
    help="weight for unimodal regression losses inside CMRF",
)
parser.add_argument(
    "--contrast",
    action="store_true",
    help="enable supervised contrastive learning over modality projections",
)
parser.add_argument(
    "--alpha_contrast",
    type=float,
...
parser.add_argument(
    "--distill",
    action="store_true",
    help="enable SMA-teacher distillation on mixed representations",
)
parser.add_argument(
    "--distill_coef",
    type=float,
    default=3.0,
    help="weight for the CMRF distillation loss",
)
parser.add_argument(
    "--sma",
    action="store_true",
    help="enable SMA teacher model updates",
)
parser.add_argument(
    "--sma_start_step",
    type=int,
    default=100,
    help="training step after which SMA becomes an average teacher",
)
parser.add_argument(
    "--contrast_temp",
    type=float,
...
parser.set_defaults(cmrf_mixup=True, contrast=True, distill=True, sma=True)
...
if __name__ == "__main__":
    if hyp_params.stage in {"dg_train", "dg_test"}:
        ...
        run_result = CMRF_Model.initiate(
            hyp_params, train_loader, valid_loader, test_loader
        )
        if not isinstance(run_result, dict):
            run_result = {"final_test_loss": float(run_result)}
        append_run_log(hyp_params, "cmrf", run_result, run_started_at=run_started_at)
    else:
        raise ValueError(
