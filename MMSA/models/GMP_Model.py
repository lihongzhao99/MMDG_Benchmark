...
def _regression_confidence(preds, targets):
    # Higher confidence for smaller MAE.
...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    for module in gmp_modules.values():
    ...
    trainable_params = _count_trainable_params(model) + sum(
        _count_trainable_params(m) for m in gmp_modules.values()
    )
    optimizer_params = _count_optimizer_params(optimizer)
    print(
        f"Trainable params (fusion+GMP): {trainable_params}, Optimizer params: {optimizer_params}"
    )
...
def _forward_mdja_components(
    ...
    local_domain_labels = None
    text_domain_logits = None
    audio_domain_logits = None
    vision_domain_logits = None
    text_domain_loss = torch.tensor(0.0, device=text.device)
    audio_domain_loss = torch.tensor(0.0, device=text.device)
    vision_domain_loss = torch.tensor(0.0, device=text.device)
    ...
    aux_reg_loss = (text_loss + audio_loss + vision_loss) / 3.0
    ...
    modal_domain_losses = {
        "text": text_domain_loss,
        "audio": audio_domain_loss,
        "vision": vision_domain_loss,
    }
    modal_preds = {
        "text": text_pred,
        "audio": audio_pred,
    ...
    domain_logits = {
        "text": text_domain_logits,
        "audio": audio_domain_logits,
        "vision": vision_domain_logits,
    }

    return {
        "preds": preds,
        "fused_loss": fused_loss,
        "domain_adv_loss_local": domain_adv_loss_local,
        "aux_reg_loss": aux_reg_loss,
        "total_loss_mdja": total_loss_mdja,
        "modal_cls_losses": modal_cls_losses,
        "modal_domain_losses": modal_domain_losses,
        "modal_preds": modal_preds,
        "domain_logits": domain_logits,
        "local_domain_labels": local_domain_labels,
    }


def _apply_gmp_local_gradients(
    model,
    mdja,
    losses,
    eval_attr,
    ...
    active_modals = ["text", "audio", "vision"]

    sem_conf = {
        m: _regression_confidence(losses["modal_preds"][m], eval_attr)
        for m in active_modals
    }
    sem_ratio = _compute_diff_ratios(sem_conf)
    sem_coeff = {
        m: _mod_coeff_from_ratio_tanh(sem_ratio[m], hyp_params.alpha_k)
        for m in active_modals
    }

    if hyp_params.enable_domain_adv:
        local_domain_labels = losses["local_domain_labels"]
        dom_conf = {
            m: _mean_confidence(losses["domain_logits"][m], local_domain_labels)
            for m in active_modals
        }
        dom_ratio = _compute_diff_ratios_domain(dom_conf)
        dom_coeff = {
            m: _mod_coeff_from_ratio_tanh(dom_ratio[m], hyp_params.alpha_p)
            for m in active_modals
        }
    else:
        dom_ratio = {
            m: torch.tensor(1.0, device=eval_attr.device) for m in active_modals
        }
        dom_coeff = {
            m: torch.tensor(1.0, device=eval_attr.device) for m in active_modals
        }

    gmp_model_grads = tuple(None for _ in model_params)

    for m in active_modals:
        g_cls = torch.autograd.grad(
            losses["modal_cls_losses"][m] * hyp_params.cls_loss,
            model_params,
            retain_graph=True,
            allow_unused=True,
        )
        ...
        if hyp_params.enable_domain_adv:
            g_dom = torch.autograd.grad(
                losses["modal_domain_losses"][m] * hyp_params.domain_adv_loss_local,
                model_params,
                retain_graph=True,
                allow_unused=True,
            )
        else:
            g_dom = tuple(None for _ in model_params)

        g_cls = _scale_grads(g_cls, sem_coeff[m])
        g_dom = _scale_grads(g_dom, dom_coeff[m])

        if hyp_params.enable_domain_adv and _dot_grads(g_cls, g_dom) < 0:
            if sem_ratio[m] >= dom_ratio[m]:
                g_cls = _project_conflict(g_cls, g_dom)
            else:
                g_dom = _project_conflict(g_dom, g_cls)

        gmp_model_grads = _add_grads(gmp_model_grads, _add_grads(g_cls, g_dom))

    for p, g in zip(model_params, gmp_model_grads):
        if g is None:
            continue
        if p.grad is None:
    ...
    local_aux = (
        hyp_params.cls_loss
        * (
            losses["modal_cls_losses"]["text"]
            + losses["modal_cls_losses"]["audio"]
            + losses["modal_cls_losses"]["vision"]
        )
        / 3.0
    )
    if hyp_params.enable_domain_adv:
        local_aux = local_aux + (
            hyp_params.domain_adv_loss_local
            * (
                losses["modal_domain_losses"]["text"]
                + losses["modal_domain_losses"]["audio"]
                + losses["modal_domain_losses"]["vision"]
            )
            / 3.0
        )

    local_mdja_params = (
        list(mdja["text_reg"].parameters())
        + list(mdja["audio_reg"].parameters())
        + list(mdja["vision_reg"].parameters())
        + list(mdja["text_proj"].parameters())
        + list(mdja["audio_proj"].parameters())
        + list(mdja["vision_proj"].parameters())
        + list(mdja["domain_disc_local"].parameters())
    )

    local_mdja_grads = torch.autograd.grad(
        local_aux,
        local_mdja_params,
        retain_graph=False,
        allow_unused=True,
    )
    for p, g in zip(local_mdja_params, local_mdja_grads):
        if g is None:
            continue
        if p.grad is None:
            p.grad = g.detach().clone()
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    enable_gmp = getattr(hyp_params, "enable_gmp", False)
    ...
    def train_epoch():
        ...
        for i_batch, batch in enumerate(train_loader):
            ...
            losses = _forward_mdja_components(
                model,
                mdja,
                criterion_reg,
                criterion_ce,
            ...
            if enable_gmp:
                base_loss = losses["fused_loss"]
                base_loss.backward(retain_graph=True)
                _apply_gmp_local_gradients(model, mdja, losses, eval_attr, hyp_params)
                combined_loss = (
                    base_loss
                    + hyp_params.domain_adv_loss_local * losses["domain_adv_loss_local"]
                    + hyp_params.cls_loss * (
                        losses["modal_cls_losses"]["text"]
                        + losses["modal_cls_losses"]["audio"]
                        + losses["modal_cls_losses"]["vision"]
                ...
                combined_loss = losses["total_loss_mdja"]
                combined_loss.backward()

            raw_loss = losses["fused_loss"]
