...
from models import MOOSA_Model
from utils.dataloader import getdataloader
from utils.run_logging import append_run_log


parser = argparse.ArgumentParser(description="MMSA + MOOSA domain generalization")
...
parser.add_argument(
    ...
    type=int,
    default=256,
    help="hidden size for SimMMDG encoder / translation heads",
)
parser.add_argument(
    "--simmmdg_proj_dim",
    type=int,
    default=128,
    help="projection dim for supervised contrastive learning",
)
parser.add_argument(
    "--alpha_trans",
    type=float,
    default=0.1,
    help="weight for cross-modal translation loss",
)
parser.add_argument(
    "--alpha_contrast",
    type=float,
...
parser.add_argument(
    ...
    type=float,
    default=0.7,
    help="weight for split-feature separation regularization",
)
parser.add_argument(
    "--contrast_temp",
    type=float,
...
parser.add_argument(
    ...
    type=float,
    default=1.0,
    help="temperature for entropy-based branch weighting",
)
parser.add_argument(
    "--entropy_min_weight",
    type=float,
    default=1e-3,
    help="weight for entropy minimization across branch classifiers",
)
parser.add_argument(
    "--jigsaw_ratio",
    type=float,
    default=1.0,
    help="weight for multimodal jigsaw loss",
)
parser.add_argument(
    "--jigsaw_num_splits",
    type=int,
    default=2,
    help="number of chunks to split each modality embedding for jigsaw",
)
parser.add_argument(
    "--jigsaw_samples",
    type=int,
    default=128,
    help="number of sampled multimodal jigsaw permutations",
)
parser.add_argument(
    "--jigsaw_hidden",
    type=int,
    default=256,
    help="hidden size for the multimodal jigsaw classifier",
)
parser.add_argument(
    "--mask_ratio",
    type=float,
    default=0.3,
    help="feature masking ratio for cross-modal translation",
)
...
if __name__ == "__main__":
    if hyp_params.stage in {"dg_train", "dg_test"}:
        ...
        run_result = MOOSA_Model.initiate(
            hyp_params, train_loader, valid_loader, test_loader
        )
        if not isinstance(run_result, dict):
            run_result = {"final_test_loss": float(run_result)}
        append_run_log(hyp_params, "moosa", run_result, run_started_at=run_started_at)
    else:
        raise ValueError(
