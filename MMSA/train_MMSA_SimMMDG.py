...
from models import SimMMDG_Model
from utils.dataloader import getdataloader
from utils.run_logging import append_run_log


parser = argparse.ArgumentParser(description="MMSA + SimMMDG domain generalization")
...
if __name__ == "__main__":
    if hyp_params.stage in {"dg_train", "dg_test"}:
        ...
        run_result = SimMMDG_Model.initiate(
            hyp_params, train_loader, valid_loader, test_loader
        )
        if not isinstance(run_result, dict):
            run_result = {"final_test_loss": float(run_result)}
        append_run_log(hyp_params, "simmmdg", run_result, run_started_at=run_started_at)
    else:
        raise ValueError(
