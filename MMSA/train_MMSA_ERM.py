...
from models import ERM_Model
from utils.dataloader import getdataloader
from utils.run_logging import append_run_log


parser = argparse.ArgumentParser(description="qingganfenxi")
...
parser.add_argument(
    "--num_epochs", type=int, default=15, help="number of epochs"
)
parser.add_argument(
...
parser.add_argument("--gpu_id", type=int, default=1, help="single GPU id to use")
parser.add_argument(
    "--multi_gpu",
    action="store_true",
...
if __name__ == "__main__":
    if hyp_params.stage in {"dg_train", "dg_test"}:
        ...
        run_result = ERM_Model.initiate(
            hyp_params, train_loader, valid_loader, test_loader
        )
        if not isinstance(run_result, dict):
            run_result = {"final_test_loss": float(run_result)}
        append_run_log(hyp_params, "main", run_result, run_started_at=run_started_at)
    else:
        raise ValueError(
