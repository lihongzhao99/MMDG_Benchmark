...
from models import NEL_Model
from utils.dataloader import getdataloader
from utils.run_logging import append_run_log


parser = argparse.ArgumentParser(description="MMSA + NEL domain generalization")
...
parser.add_argument("--temp_s", type=float, default=0.1, help="temperature for supervised contrastive")
parser.add_argument("--temp_u", type=float, default=0.25, help="temperature for mean-shift contrastive")
parser.add_argument("--k", type=int, default=8, help="k for mean-shift nearest neighbors")
parser.add_argument("--nel_proj_dim", type=int, default=128, help="projection dim for NEL head")

args = parser.parse_args()

if args.beta is None:
    args.beta = 1.0 / args.batch_size
...
if __name__ == "__main__":
    if hyp_params.stage in {"dg_train", "dg_test"}:
        ...
        run_result = NEL_Model.initiate(
            hyp_params, train_loader, valid_loader, test_loader
        )
        if not isinstance(run_result, dict):
            run_result = {"final_test_loss": float(run_result)}
        append_run_log(hyp_params, "nel", run_result, run_started_at=run_started_at)
    else:
        raise ValueError(
