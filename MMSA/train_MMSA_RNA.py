...
from models import RNA_Model
from utils.dataloader import getdataloader
from utils.run_logging import append_run_log


parser = argparse.ArgumentParser(description="MMSA + RNA domain generalization")
...
parser.add_argument(
    "--alpha_RNA", type=float, default=1.0, help="weight for RNA loss (default: 1.0)"
)
...
if use_cuda and (not args.multi_gpu):
    # Force single-GPU training/inference.
...
hyp_params.device = torch.device(
    f"cuda:{args.gpu_id}" if use_cuda else "cpu"
)
...
if __name__ == "__main__":
    if hyp_params.stage in {"dg_train", "dg_test"}:
        ...
        run_result = RNA_Model.initiate(
            hyp_params, train_loader, valid_loader, test_loader
        )
        if not isinstance(run_result, dict):
            run_result = {"final_test_loss": float(run_result)}
        append_run_log(hyp_params, "rna", run_result, run_started_at=run_started_at)
    else:
        raise ValueError(
